...
async def product_catalog_keyboard(
    products: list, parent_id: int, category_id: int, page: int, max_page: int, filters: bool = False
):
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton(text="Фильтрация 🔍", callback_data=f"settings_filters:{category_id}"))
    for product in products:
        keyboard.add(
            InlineKeyboardButton(text=product["name"], callback_data=f"product:{product['id']}:desc:0:{page}:cat")
        )
...
async def product_keyboard(
    ...
    prev_photo = (
        InlineKeyboardButton(
            text="⬅️", callback_data=f"list_photo:{product_id}:{desc_or_attr}:{photo_index-1}:{page}:{catalog_or_cart}"
        )
        if photo_index > 0
        else None
    )
    ...
    next_photo = (
        ...
        if photo_index + 1 < len(photo_list)
        else None
    )
    ...
    if favorite:
        favorite_button = InlineKeyboardButton(
            text="Из избранного ⭐",
            callback_data=f"remove_favorite:{product_id}:{desc_or_attr}:{photo_index}:{page}:{catalog_or_cart}",
        )
    else:
        favorite_button = InlineKeyboardButton(
            text="В избранное ⭐",
            callback_data=f"add_favorite:{product_id}:{desc_or_attr}:{photo_index}:{page}:{catalog_or_cart}",
        )

    if not cart:
        add_to_cart_button = InlineKeyboardButton(
            text="В корзину 🛒",
            callback_data=f"add_in_cart:{product_id}:{desc_or_attr}:{photo_index}:{page}:{catalog_or_cart}",
        )
        ...
        minus_button = (
            InlineKeyboardButton(
                text="➖", callback_data=f"minus_c:{product_id}:{desc_or_attr}:{photo_index}:{page}:{catalog_or_cart}"
            )
            if cart[0].quantity > 0
            else None
        )
        quantity_button = InlineKeyboardButton(
            text=f"{cart[0].quantity} шт. 🛒", callback_data=f"add_in_cart:{product_id}:{page}:{catalog_or_cart}"
        )
        plus_button = (
            InlineKeyboardButton(
                text="➕", callback_data=f"plus_c:{product_id}:{desc_or_attr}:{photo_index}:{page}:{catalog_or_cart}"
            )
            if cart[0].quantity < quantity_max
            else None
        )
    ...
    if desc_or_attr == "desc":
        attr_desc_button = InlineKeyboardButton(
            text="Характеристики товара 📜",
            callback_data=f"product:{product_id}:attr:{photo_index}:{page}:{catalog_or_cart}",
        )
    elif desc_or_attr == "attr":
        attr_desc_button = InlineKeyboardButton(
            text="Описание товара 📜", callback_data=f"product:{product_id}:desc:{photo_index}:{page}:{catalog_or_cart}"
        )

...
async def process_shipping_query(shipping_query: types.ShippingQuery):
    ...
    if shipping_query.shipping_address.country_code != "RU":
        return await bot.answer_shipping_query(
            shipping_query.id, ok=False, error_message="Отправление товара происходит только по территории РФ."
        )
    if (
        shipping_query.shipping_address.post_code.isdigit() is False
        or len(shipping_query.shipping_address.post_code) != 6
    ):
        return await bot.answer_shipping_query(
            shipping_query.id, ok=False, error_message="Почтовый индекс состоит из 6 цифр."
        )
    resp = await api.pochta_rf(postcode=shipping_query.shipping_address.post_code, weight=weight)
    if "detail" in resp:
        return await bot.answer_shipping_query(shipping_query.id, ok=False, error_message=resp["detail"])
...
async def process_successful_payment(message: types.Message):
    ...
    if pmnt.invoice_payload.isdigit():
        ...
        resp = await api.create_pp_order(
            order_amount=pmnt.total_amount / 100,

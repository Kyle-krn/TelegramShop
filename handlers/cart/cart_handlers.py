...
async def cart_handler(message: Message or CallbackQuery):
    ...
    keyboard = await main_cart_handler(products=products, user_cart=user_cart, page=page, max_page=max_page)
    kwargs = {"text": f"В вашей коризне {user_cart_count} товаров.", "reply_markup": keyboard}
    if edit_product:
        kwargs["text"] += "\n\n" + edit_product
...
async def checkout_in_cart_handler(call: CallbackQuery):
    ...
    keyboard = await main_cart_handler(products=products, user_cart=user_cart, page=1, max_page=1, checkout=True)
...
async def checkouts_product(call: CallbackQuery):
    ...
    product_in_cart = await UserCart.get(
        Q(user__tg_id=call.message.chat.id) & Q(product_id=int(call.data.split(":")[1]))
    )

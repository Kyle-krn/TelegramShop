...
async def boolean_filter_handlers(call: CallbackQuery):
    ...
    attr_in_data = (
        [i for i in search_data.attrs if i["name"] == name_attr.string] if search_data.attrs is not None else None
    )
    if attr_in_data:
    ...
    if len(attr_in_category) == 0:
        return await call.message.edit_text(
            text=f"Атриубт {name_attr.string} был удален.",
    ...
    try:
        await call.message.edit_text(
            text=f"<b>{name_attr.string}</b>",
            reply_markup=await boolean_keyboard(attr_name_id=name_attr.id, value=attr_in_data, category_id=category_id),
        )
...
async def bool_yes_handler(call: CallbackQuery):
    ...
    if call.data.split(":")[0] == "bool_true":
        ...
        search_data.attrs = (
            [i for i in search_data.attrs if i["name"] != name_attr] if search_data.attrs is not None else None
        )
    ...
    if attr_in_data == []:
        ...
        attrs = [{"name": name_attr, "value": value, "min": None, "max": None}] + [
            i for i in search_data.attrs if i["name"] != name_attr
        ]

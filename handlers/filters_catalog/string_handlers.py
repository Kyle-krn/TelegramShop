...
async def string_filter_handler(call: CallbackQuery):
    ...
    attr_in_data = (
        [i for i in search_data.attrs if i["name"] == name_attr.string] if search_data.attrs is not None else []
    )
    if attr_in_data:
    ...
    if distinct_values == [None] or distinct_values == []:
        ...
        return await call.message.edit_text(
            text=f"У атриубта {name_attr.string} не задано параметров.",
            reply_markup=await back_keyboard(callback=f"settings_filters:{category_id}"),
        )

    keyboard = await string_keyboard(
        attr_name_id=name_attr.id,
        category_id=category_id,
        distinct_value_list=distinct_values,
        user_values=attr_in_data,
    )

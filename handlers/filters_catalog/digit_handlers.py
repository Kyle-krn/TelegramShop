...
async def digit_filter_handler(call: CallbackQuery):
    ...
    attr_in_category = [i for i in category['filters'] if i['name'] == attr_name]
    ...
    if type(attr_in_category[0]['value']) not in [int, float]:
        raise IndexError(f"Атрибут {attr_name} не является числом.")
    min_max = await api.get_min_max_digit_attr(category_id=category_id, attr_name=attr_name)
    attr_in_data = [i for i in search_data.attrs if i['name'] == attr_name] if search_data.attrs is not None else []
    if attr_in_data:
        ...
        attr_in_data = {'name': attr_name, 'value': None, 'min': min_max['min'], 'max': min_max['max']}
        if type(attr_in_category[0]['value']) == int:
            attr_in_data['min'] = int(attr_in_data['min'])
            attr_in_data['max'] = int(attr_in_data['max'])
    
    keyboard = await digit_keyboard(attr_name=attr_name, 
...
async def min_price_handler(call: CallbackQuery):  
    ...
    await call.message.edit_text(text=text, reply_markup=await back_keyboard(callback=f"cds:digit_filter:{kwargs['attr_name']}:{kwargs['category_id']}"))
...
async def process_price_invalid(message: Message, state: FSMContext):
    ...
    attr_in_category = [i for i in category['filters'] if i['name'] == user_data['attr_name']]
    try:
    ...
    min_max = await api.get_min_max_digit_attr(category_id=user_data['category_id'], attr_name=user_data['attr_name'])
    ...
    attr_in_data = [i for i in search_data.attrs if i['name'] == user_data['attr_name']] if search_data.attrs is not None else []
    if not attr_in_data:
        attr_in_data = [{'name': user_data['attr_name'], 'value': None, 'min': min_max['min'], 'max': min_max['max']}]
    ...
    search_data.attrs = [i for i in search_data.attrs if i['name'] != user_data['attr_name']] + attr_in_data if search_data.attrs else attr_in_data
    await search_data.save()
    await state.finish()

    return await message.answer(text=text + "Что бы продолжить нажмите кнопку ниже 👇", 
                                reply_markup=await back_keyboard(callback=f"digit_filter:{user_data['attr_name']}:{user_data['category_id']}", 

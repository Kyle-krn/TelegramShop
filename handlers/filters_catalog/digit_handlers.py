...
async def digit_filter_handler(call: CallbackQuery):
    ...
    if len(attr_in_category) == 0:
        ...
        return await call.message.edit_text(
            text=f"Атриубт {attr_name.string} был удален.",
    ...
    if min_max["max"] is None or min_max["min"] is None:
        ...
        return await call.message.edit_text(
            text=f"У атриубта {attr_name.string} не задано параметров.",
            reply_markup=await back_keyboard(callback=f"settings_filters:{category_id}"),
        )
    attr_in_data = (
        [i for i in search_data.attrs if i["name"] == attr_name.string] if search_data.attrs is not None else []
    )
    if attr_in_data:
    ...
    keyboard = await digit_keyboard(
        attr_name_id=attr_name.id,
...
async def min_price_handler(call: CallbackQuery):
    ...
    kwargs = {"attr_name_id": call.data.split(":")[1], "category_id": int(call.data.split(":")[2])}
    await state.update_data(**kwargs)
    await call.message.edit_text(
        text=text,
        reply_markup=await back_keyboard(callback=f"cds:digit_filter:{kwargs['attr_name_id']}:{kwargs['category_id']}"),
    )
...
async def process_price_invalid(message: Message, state: FSMContext):
    ...
    if (min_max["min"] <= value <= min_max["max"]) is False:
        ...
        return await message.reply(
            f"От {min_max['min']} {attr_in_category[0]['prefix']} до  {min_max['max']} {attr_in_category[0]['prefix']}",
    ...
    search_data.attrs = (
        [i for i in search_data.attrs if i["name"] != attr_name.string] + attr_in_data
    ...
    return await message.answer(
        text=text + "Что бы продолжить нажмите кнопку ниже 👇",
        reply_markup=await back_keyboard(
            callback=f"digit_filter:{attr_name.id}:{user_data['category_id']}", text="К настройкам поиска 🔎"
        ),
    )
